...
class camera_test_activity : AppCompatActivity() {
    ...
    private fun getMultiHandLandmarksDebugString(
        multiHandLandmarks: List<NormalizedLandmarkList>): String {
        if (multiHandLandmarks.isEmpty()) {
            return "No hand landmarks"
        }
        val datas: MutableList<Float> = mutableListOf()
        val data_list: MutableList<MutableList<Float>> = mutableListOf()
        ...
        for ((handIndex, landmarks) in multiHandLandmarks.withIndex()) {
            ...
            for ((landmarkIndex, landmark) in landmarks.landmarkList.withIndex()) {
                ...
                datas.add(landmark.x)

                datas.add(landmark.y)

                datas.add(landmark.z)
            }
        }
        data_list.add(datas)
        println("datas : ")
