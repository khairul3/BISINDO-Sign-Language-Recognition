...
def predict_cloud_function(datas):
    url = "https://us-central1-valid-dragon-312515.cloudfunctions.net/predict-bisindo"
    instances = np.array(datas)
    payload = json.dumps({"instances": instances.tolist()})
    ...
    min_detection_confidence=0.1) as hands:
        ...
        if isCapture and timeDelay  :
            ...
            if save_or_no == ord(' '):
                ...
                time.sleep(1)
            isCapture = False
            #temp_2 = pd.DataFrame(temp[2:])
            #print(predict_cloud_function(temp_2.T))
